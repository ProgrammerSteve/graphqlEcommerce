...
interface AlphabetizeSortProps {
  handleSortOption: (value: string) => void;
  sortOption: string;
}

const AlphabetizeSort: React.FC<AlphabetizeSortProps> = ({ handleSortOption, sortOption }) => {
  return (
    <div className="bg-gray-800 py-2 md:py-4  px-2 lg:px-3 xl:px-4  rounded-lg border-solid border-2 border-gray-600 flex items-center justify-center gap-2">
      ...
      <div className="w-4 flex flex-col justify-between py-2">
        <UpArrow
          handler={() => handleSortOption(SORT_OPTIONS.ALPHABETICALLY_ACCENDING)}
          selected={sortOption === SORT_OPTIONS.ALPHABETICALLY_ACCENDING}
        />
        <DownArrow
          handler={() => handleSortOption(SORT_OPTIONS.ALPHABETICALLY_DECENDING)}
