...
const PriceSort: React.FC<PriceSortProps> = ({ handleSortOption, sortOption }) => {
  return (
    <div className="bg-gray-800 px-2 lg:px-3 xl:px-4  py-2 md:py-4  rounded-lg border-solid border-2 border-gray-600 cursor-pointer  grid place-items-center">
      <div className="flex justify-between gap-2 px-2">
        ...
        <div className="w-4 flex flex-col justify-between py-2">
          <UpArrow
            handler={() => handleSortOption(SORT_OPTIONS.PRICE_ACCENDING)}
            selected={sortOption === SORT_OPTIONS.PRICE_ACCENDING}
          />
          <DownArrow
            handler={() => handleSortOption(SORT_OPTIONS.PRICE_DECENDING)}
